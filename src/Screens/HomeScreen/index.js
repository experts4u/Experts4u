...
export default function () {
          ...
          <TouchableWithoutFeedback
            onPress={() => {
              Navigation.navigate(Routes.ServiceDetailsScreen, {
                itemId: item?.ChildCatIDs[0]?._id,

                PCGroup: item?.ChildCatIDs[0]?.PCName[0]?.PCGroup,
                pcId: item?.PCatId,
                PCName: item?.ChildCatIDs[0]?.PCName[0]?.PCName,
                serviceId: item?._id,
              });
            }}>
            <View>
  ...
  const rendershortcuItem = ({item, index}) => (
    <CustomCardd
      index={index}
      item={item.serviceID}
      txt={item.serviceID?.ServiceName}
      img={item.serviceID?.ServiceImage}
    />
  );
          ...
          if (lastImage?.find(image => image._id === item?.id)?.serviceID) {
            Navigation.navigate(Routes.ServiceDetailsScreen, {
              itemId: lastImage?.find(image => image._id === item?.id)?.CCatId
                ?._id,
              PCGroup: lastImage?.find(image => image._id === item?.id)?.PCatId
                ?.PCGroup,
